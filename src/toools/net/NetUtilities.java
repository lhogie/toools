...
public class NetUtilities
{
	...
	public static byte[] retrieveURLContent(String cgiAddress, Map<String, String> parms, byte[] postData) throws IOException
	{
		String rq = "";
		...
		if (!parms.isEmpty())
		{
			for (String key : parms.keySet())
			{
				...
				rq += (rq.isEmpty() ? "?" : "&") + URLEncoder.encode(key, "UTF-8") + "=" + URLEncoder.encode(value, "UTF-8");
			}
		}
	...
	public static Socket connect(InetAddress ip, int port, int timeoutms) throws IOException
	{
		...
		{
			SocketTimeoutException c = new SocketTimeoutException(e.getMessage() 
					+ " to host " + ip.getHostName() + " on port " + port);
			c.setStackTrace(e.getStackTrace());
			throw c;
		}
		catch (ConnectException e)
		{
			ConnectException c = new ConnectException(e.getMessage() + " to host " + ip.getHostName() + " on port " + port);
			c.setStackTrace(e.getStackTrace());
			throw c;
		}
		return s;
	...
	public static boolean waitUntilServerStarts(InetAddress host, int port, int testTimeoutMs, int globalTimeoutMs, ConnectionCloser closer)
	{
...
//	}
//
//	public static void sendEmailUsingLocalSMTPServer(String sender, String recipient, String subject, String content) throws MessagingException
//	{
//		System.setProperty("mail.smtp.starttls.enable", "true");
//		System.setProperty("mail.smtp.host", "localhost");
//		Session session = Session.getDefaultInstance(System.getProperties(), null);
//		MimeMessage message = new MimeMessage(session);
//		message.setFrom(new InternetAddress(System.getProperties().getProperty("user.name")));
...
//	}
//
//	public static void sendEmail(String sender, String recipient, String subject, String content, String smtpServer, String username, String passwd)
...
//	}
	...
	public static Collection<Socket> getWorkingServers(final int port, Collection<InetAddress> candidateServers, final int timeoutms)
	{
	...
	{
		for (NetworkInterface ni : getNICs().getKeys())
		{
	...
	public static Relation<NetworkInterface, InetAddress> getNICs()
	{
		if (networkInterfaces == null)
		{
			...
			try
			{
				for (NetworkInterface netInterface : Collections.convertEnumerationToList(NetworkInterface.getNetworkInterfaces()))
				{
					networkInterfaces.addAll(netInterface, Collections.convertEnumerationToList(netInterface.getInetAddresses()));
				}
			}
	...
	public static Set<NetworkInterface> getHardwareNetworkInterfaces()
	{
		...
		for (NetworkInterface nic : getNICs().getKeys())
		{
			if (isHardware(nic))
			{
				r.add(nic);
	...
	public static Set<InetAddress> getHardwareIPv4Addresses()
	{
		...
		for (NetworkInterface nic : nic_ips.getKeys())
		{
			if (isHardware(nic))
			{
	...
	public static InetAddress getVPNAddress()
	{
		...
		for (NetworkInterface ni : nic_ips.getKeys())
		{
			if (!isHardware(ni))
			{
	...
	public static ServerSocket findFreePort(int basePort) throws IOException
	{
